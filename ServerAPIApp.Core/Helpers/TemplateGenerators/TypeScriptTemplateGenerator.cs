...
{
    public class TypeScriptTemplateGenerator : ITemplateGenerator
    {
        public string LanguageCode => "typescript";
        ...
        public string BuildTemplate(ManifestDto manifest)
        {
            ...
            var entrypoint = NormalizeMethodName(manifest.Entrypoint);

            var parameterList = string.Join(", ",
                parameters.Select((p, i) =>
                    $"{NormalizeIdentifier(p.Name, $"arg{i}")}: {TypeScriptTypeNameRenderer.RenderTypeName(p.Type)}"));

            var returnType = TypeScriptTypeNameRenderer.RenderTypeName(signature?.ReturnType);

            var sb = new StringBuilder();

            sb.Append($"function {entrypoint}({parameterList})");
